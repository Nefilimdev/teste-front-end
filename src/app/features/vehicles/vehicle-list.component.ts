...
export class VehicleListComponent {
  ...
  protected readonly totalPages = computed(() =>
    Math.max(1, Math.ceil(this.totalVehicles() / this.pageSize))
  );
  ...
  protected readonly rangeStart = computed(() =>
    this.totalVehicles() === 0 ? 0 : (this.currentPage() - 1) * this.pageSize + 1
  );
  protected readonly rangeEnd = computed(() =>
    Math.min(this.currentPage() * this.pageSize, this.totalVehicles())
  );
